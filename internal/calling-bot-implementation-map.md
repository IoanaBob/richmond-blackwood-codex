...
- `RB Calls Voice Execution` active version `4db1b274-d26f-40bb-9519-b0699a324ae7` now keeps ElevenLabs startup context small and plain text: direct Company/Individual/Contact summaries, tax registration/reference context, owner/routing metadata, representative identity variables, live-help sentinels, and a public-safe voice `context_pack`. Detailed non-startup records for tax filings, contracts, correspondence, bank accounts, tax payments/prepayments, assets, and call notes are fetched during the call through `RB Calls Context Lookup`. n8n preserves raw identifiers only and reinforces that they must be spoken much, much slower with clear breaks; pronunciation remains controlled by the ElevenLabs prompt. File fields are not sent as file names, URLs, or content; only metadata such as file-property counts may be retained for coverage checks. The lock claim step clears stale `ElevenLabs Conversation ID` and `Twilio Call SID` before a new outbound attempt so a canceled n8n execution cannot leave the call pointing at an old conversation. The voice-payload builder reads upstream nodes through raw `$items(...)` access rather than paired-item ancestry from `Build Linked Record Queue`. The outbound ElevenLabs endpoint defaults to SIP trunking and can be rolled back to Twilio Native by setting n8n variable `ELEVENLABS_OUTBOUND_CALL_PROVIDER=twilio`.
...
- `RB Calls ElevenLabs Events` active version `c209a5fb-905c-4aa9-877e-9cd1f5ea4d37` includes a one-minute no-answer watchdog. Started calls older than two minutes with zero messages and no pickup are moved to `Call Unanswered` and get a Call Note without posting Slack status noise, including ElevenLabs states `initiated` and `in-progress` when duration remains zero. Claim-only locks with no stored conversation ID are treated as stale after two minutes and swept to follow-up, which covers canceled/manual n8n executions after lock claim. IVR dead-ends that end before a live agent are treated as follow-up/unanswered even if ElevenLabs marks the conversation successful. The conversation status check uses the same n8n predefined `ElevenLabs API` credential as outbound calling.
...
- On 2026-05-21, the first `RBCALL-18` Brussels Airlines / Miles & More retry showed that in-band DTMF reached the privacy/live-agent prompt, but the agent was still speaking around IVR turns and the first greeting collided with the remote menu. A later blank-first-message attempt created ElevenLabs conversation `conv_5801ks54jgtxfypsw4rgg5gnavkv` and Twilio SID `CA40390f109bb0173aa26939c681b7ee72`, but it stayed `in-progress` with duration `0s`, no transcript, no tool calls, and no dynamic variables exposed. `RB Calls Voice Execution` now removes that empty `conversation_config_override.agent.first_message` override while keeping per-call language override. The live RB Call Bot version `agtvrsn_9201ks5mf0ngfryt1ae3vw1h94gp` sets keypad tools to SIP out-of-band D-T-M-F for future SIP trunk calls; on Twilio Native this flag is ignored by ElevenLabs. Do not use an empty first-message override for IVR contacts. For Brussels Airlines, the direct Twilio Native ElevenLabs path remains fragile; prefer SIP trunking or Twilio-native IVR pre-navigation/bridge before further production retries.
...
- First message: default English opener is `Hello, my name is Alexander Gulin. I am calling regarding an administrative matter for a client and would like to make sure I am speaking with the right department.` German calls use the German ElevenLabs language-preset opener. Do not send blank first-message overrides for IVR contacts; the 2026-05-21 Brussels retry showed that this can create a zero-audio stuck conversation. IVR mitigation uses post-tone speech suppression and conservative prompt routing; SIP trunk calls use ElevenLabs out-of-band D-T-M-F, while Twilio Native ignores that flag.
...
- Prompt now includes `Pronunciation And Spelling`, `Identifier Pronunciation - Prompt Controlled`, and `Slow Number Delivery - Hard Rule`: n8n sends raw identifiers only; the agent must preserve exact raw values while saying numbers, registration numbers, tax references, phone numbers, and alphanumeric strings much, much slower than normal speech in the current call language. English calls are as important as German calls. Abbreviations are hyphenated letter runs in all languages (`V-A-T`, `P-O-A`, `U-B-O`, `R-B-O`, `E-O-R-I`, `P-P-S-N`, `U-T-R`, `T-A-I-N`, `R-O-S`, `H-M-R-C`, `C-R-O`, `I-D`, `R-B`) and are never pronounced as words. German calls use German digit words (`null`, `eins`, `zwei`, `drei`, `vier`, `fünf`, `sechs`, `sieben`, `acht`, `neun`) and German letter names. If the authority is confused by letters, use NATO for English, German Buchstabiertafel for German, or the closest recognized spelling alphabet for the call language. Identifier examples use commas for short numeric grouping and periods for longer breaks, with no literal `pause` words and no ellipses.
...
- The agent prompt now includes IVR/menu handling rules. It distinguishes IVR from voicemail, uses `play_keypad_touch_tone` only for clear menu choices, sends keypad tones in-band, avoids entering sensitive identifiers unless present in context, and records the menu path/outcome.
...
- IVR handling depends on the model selecting the visible IVR branch and the remote PSTN/IVR accepting the keypad tones. The current agent uses in-band DTMF because Brussels Airlines did not reliably accept the previous Twilio out-of-band mode. For Brussels Airlines specifically, do not keep retrying the same direct ElevenLabs path as the production solution; build or test Twilio-native IVR pre-navigation/bridging, or use an alternate official contact route.
...
- Active version: `4db1b274-d26f-40bb-9519-b0699a324ae7`.
...
16. Calls the ElevenLabs outbound-call endpoint selected by n8n variable `ELEVENLABS_OUTBOUND_CALL_PROVIDER`: `https://api.elevenlabs.io/v1/convai/sip-trunk/outbound-call` by default, or Twilio Native only when the provider is explicitly `twilio`.
...
- n8n variable: `ELEVENLABS_OUTBOUND_CALL_PROVIDER`; leave blank or set to `sip_trunk` for ElevenLabs SIP-trunk outbound calls backed by Twilio Elastic SIP Trunking. Set to `twilio` only for rollback to the older Twilio Native endpoint.
...
7. `Create Post-Call Note` and `Create Swept Status Note` use native Notion create-page nodes with the `Call` relation as an array expression, `[$json.call_page_id]`, to avoid n8n's `relationValue.filter is not a function` failure and to avoid broken HTTP Request Notion credentials.
...
12. The status-check HTTP node uses the n8n predefined `ElevenLabs API` credential. If that credential is missing or unauthorized, the sweep skips outcome mutation rather than falsely marking a call unanswered.
...
`RB Calls Voice Execution` active version `4db1b274-d26f-40bb-9519-b0699a324ae7` stops crawling linked records before call start. ElevenLabs startup dynamic variables are limited to direct tax registration/reference context, Contact/Company/Individual summaries, owner/routing metadata, live-help sentinels, and the public-safe call brief. It no longer sends correspondence, tax filings, contracts, bank accounts, tax payments/prepayments, assets, or call notes as startup context.
...
3. Configure n8n runtime secrets and variables:
   ...
   - For Twilio SIP trunking, import the SIP number into ElevenLabs and set `ELEVENLABS_AGENT_PHONE_NUMBER_ID` to that SIP phone-number ID. The workflow defaults to SIP trunking; set `ELEVENLABS_OUTBOUND_CALL_PROVIDER=twilio` only for rollback.
