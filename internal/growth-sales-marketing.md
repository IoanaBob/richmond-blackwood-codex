...
- Reddit is direct community engagement first. Do not pursue Reddit moderator outreach, sponsorships, paid posts, cold/proactive DMs, modmail, or commercial counterparties, and do not create Business Partners from Reddit routes unless the user explicitly re-enables that channel. Overt promotional posting stays blocked. A narrow soft "can help in DM if useful" line may be previewed only for directly relevant US/Germany tech-comp, remote-work, or Germany-compliant setup threads, must be labelled as a promotion-rule risk, and needs exact approval. Reactive Reddit DMs are allowed only after someone engages with our public post/comment or sends an inbound DM first; they stay in Growth Targets and Growth Messages and require exact approval plus immediate Ioana Reddit-session verification before sending.
...
- Reddit daily sourcing target: find 10 recent relevant candidates for the active audience per day. This is a sourcing/review target, not a posting target. If fewer than 10 recent relevant candidates exist, report a sourcing gap and do not fill it with stale or weak-fit threads.
