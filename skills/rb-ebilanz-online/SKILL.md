...
4. Prepare a platform import workbook only from source-backed account balances:
   ...
   - Prefer a clean single worksheet named `Import` with columns `Konto`/`Account`, `Beschriftung`/`Description`, and `Saldo`/`Balance`; include debit/credit marker only if the platform/source format requires it.
   - For account-balance uploads, use simple platform-readable `.xlsx` files first. If the upload widget is unstable, use a short ASCII filename and, if necessary, a legacy Excel `.xls` generated by a real Excel-compatible save path. Verify the sheet appears in the platform before continuing.
