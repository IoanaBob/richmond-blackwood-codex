...
After approval, send directly through the supported Gmail connector/API path. Ask whether the exact sent email should be recorded in canonical Communications (`https://www.notion.so/1b5e4130131480ab84f3cca356736807`); record it only if the user explicitly says yes and the email is a necessary third-party outbound audit/source record. If direct send is unavailable and the user approves a fallback, create a verified Gmail draft with the helper below.
