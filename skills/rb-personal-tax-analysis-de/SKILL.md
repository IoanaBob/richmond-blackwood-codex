...
- For self-employed/freelance cases, use the business/EÜR daily route (`home-office-daily-business`, group `business-betriebsausgaben`) rather than the employment `Anlage N` home-office row. Keep the amount at `Needs operator review` until the qualifying business home-office day count is confirmed; business travel days should be excluded or separately justified before claiming. When the operator confirms a full-year home-office assumption except business travel, drive `Units / Days` from the tax-year day count less unique `Expenses` rows categorised as `business-travel`, then let the statutory cap limit the included amount.
