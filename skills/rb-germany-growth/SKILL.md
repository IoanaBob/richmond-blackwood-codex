...
- Resume-with-unfinished-channel rule: when the user says `continue`, `resume`, or restarts a run and any channel has an unresolved approval, blocker, missing send proof, or unprinted packet from the current or immediately prior master run, reconcile that unfinished channel before moving to a later channel. A `Waiting Approval` row is not finished if the user is expecting that channel to be completed; either get the exact approval, perform the approved action with proof, or mark the item explicitly blocked/skipped with the reason before advancing.
...
6. Draft Packet
   ...
   - For outbound growth copy, avoid the word "path," slash-heavy phrasing, and mechanical contrast sentences that read like a template.
   - Never use enumerations in outbound growth copy. Do not use numbered/list syntax, first/second/third phrasing, category piles, issue piles, chained alternatives, or comma-separated lists of examples. Reduce the message to the single concrete point that matters, or rewrite it as natural prose if two facts are genuinely needed.
   ...
   - Do not stack examples, alternatives, caveats, or issue lists in outward-facing text. Use one concrete point, or two only when the message truly needs both, written as natural prose rather than a list.
