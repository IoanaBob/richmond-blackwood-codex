...
- Stage 12 Slack copy must group business updates by destination and task linkage, in this order: `Communications without tasks`, `Communications with tasks`, `Expenses`, `Invoices`, `Needs checking`, `Replies coming up`, and `Blocked`. Do not use one long undifferentiated `Incoming handled` list.
...
- Slack source-message marker rule: a `📝` reaction on the source Slack message means the message has been logged/triaged into RB records and should not be processed again. Do not use `📝` as a generic acknowledgement.
- When a Slack source message is triaged, post a source-thread reply with the destination task or record links. Preferred wording: `Triaged into: <task/record links>.` If no task is needed, use `Logged into: <Communication/Expense/Invoicing row>; no task needed.` Do not include broad channel mentions or new action requests in this marker reply.
...
Packet must include run ID, run window, source systems, canonical Communications DB, RB Client Databases page, task registry version, and prior incomplete run state.
...
Propose source markers only after records, tasks, and replies are correct.

Allowed source markers:

- Gmail `Triaged` labels.
- WhatsApp checkpoint changes.
- Slack source-message `📝` reactions plus a thread reply that states which approved task or record the message was triaged into.

For Slack marker rows, include source Slack message URL, channel/thread, exact reaction (`📝` / `:memo:`), exact thread-reply text, linked Communication, linked task or operational row, and skip logic for messages that already have `📝` or an equivalent thread reply.

For prior bad triage, keep existing Gmail labels and add corrective packet notes.

Auto-approved stage: write and print the packet, then continue directly to Stage 11 unless the plan includes an unexpected source mutation outside Gmail labels, WhatsApp checkpoints, or the limited Slack marker pattern above.

### 11. Source Marker Results

Apply approved Gmail labels, WhatsApp checkpoints, and Slack source-message markers.

Packet lists marked Gmail messages, skipped Gmail messages, WhatsApp checkpoint state, Slack messages reacted with `📝`, Slack thread replies posted, skipped Slack markers, and failures.

Auto-approved stage: write and print the packet, then continue directly to Stage 12 unless marking failed, a route/checkpoint blocker needs operator review, or a Slack marker would exceed the limited source-marker pattern.
...
Slack body section order:

1. `Communications without tasks` - logged or updated Communications that do not carry a task relation and still matter to humans.
2. `Communications with tasks` - Communications linked to active tasks or owner action paths. Every bullet in this section must include the linked task URL(s) by name. If the task route is not reliable and human-useful, do not put the Communication in this section.
3. `Expenses` - new or updated Expense rows, including upload/review blockers.
4. `Invoices` - new or updated Invoicing rows only. Use `None` when no invoice row changed; do not bury invoice-row changes in communication bullets.
5. `Needs checking` - concrete owner checks with resolved Slack user IDs where available.
6. `Replies coming up` - only replies that are actually due, snoozed, or waiting on listed task checks.
7. `Blocked` - evidence, access, upload, translation, or route blockers linked to the owning row.

Slack preview requirements:

- Hyperlink every item in the communication, expense, invoice, reply, and blocker sections.
- In `Communications with tasks`, hyperlink both the Communication and every linked task mentioned in the bullet.
...
- Do not include a Communication in the human-facing Slack body only to say it was routed somewhere if the operator disputes that route or the route is not useful for team action; keep that uncertainty in the packet and revise the record in a separate approved update stage if needed.
...
Packet summarizes sources checked, all DB rows created/updated, tasks closed/advanced, owner actions, replies, Gmail labels, WhatsApp checkpoints, Slack `📝` source markers/thread replies, blockers, and next run focus.
...
Auto-approved after Stage 12 send approval and successful Stage 13: write and print the final closeout packet, release the lock, preserve scratch packets by default for audit/recovery, and do not wait for separate operator approval. If Stage 14 lists only bounded media/evidence cleanup blockers that can be resolved without new judgment, continue directly to Stage 15.

### 15. Post-Closeout Media Evidence Cleanup

Use this stage only for cleanup of blockers already listed in Stage 14, such as media that failed to download during the main run, evidence files that can now be uploaded to an already-resolved Drive destination, or Communication rows that can be marked `Logged` because recovered media confirms the route.
...
- changing source markers, Gmail labels, WhatsApp checkpoints, or Slack `📝` reactions/thread replies;
