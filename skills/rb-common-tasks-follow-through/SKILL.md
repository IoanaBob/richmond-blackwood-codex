...
- Google Cloud payment receipts are business-scoped finance receipts. If no active contract or matching Invoicing row applies, create/update an Expense row with the Google Cloud receipt evidence and use `Review Required` or the equivalent Expense status when the exact company, ownership, or bookkeeping category still needs finance review. Do not create a standalone Central Task just to decide receipt ownership/routing.
...
- Revenue MyEnquiries contact notices that do not identify the client or matter in the email should create/update an internal follow-up task assigned to Johnpaul Okolie, keep Company/Individual empty until Revenue portal or related context identifies the matter, and be included in the approved `#rb-client-updates` closeout. Do not block the next stage solely because the email body lacks the client/matter subject.
...
- If the item is a Google Cloud payment receipt, treat it as an Expense-route receipt by default when no contract/Invoicing match applies. Use an Expense `Review Required` state for unresolved ownership/category rather than a separate Central Task.
