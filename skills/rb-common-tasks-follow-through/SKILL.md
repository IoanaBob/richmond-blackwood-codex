...
- For client bookkeeping/VAT source-context corrections, including no-contract/no-sales-period replies, missing-invoice scope clarification, personal-expense classification, vague transaction classification, or payment explanations such as VUN/DAPPRADAR, do not create a standalone Central Task. Attach the source context to the active recurring bookkeeping task, active VAT/source-review task, filing-dependent bookkeeping row, Communication, or owning operational row. If a prior task says it is a duplicate, or its notes say not to create/use a duplicate task, propose archiving that stale task after moving the context to the owning row; do not keep routing fresh context to the stale duplicate merely because search finds it first.
