...
| Expenses | `https://www.notion.so/3a4d858161af4519b42340242cb70351` / `collection://4648c99e-0e6e-4d17-a408-88a08017e5b0` | Primary triage destination for receipt/expense/payment-notification items. Use when no active contract/invoice route applies. If business scope is supported but final bookkeeping classification is unclear, create/update the Expense with review-required status rather than creating a Central Task. Google Cloud payment receipts are explicitly included in this Expenses route when no contract/Invoicing match applies; unresolved ownership/category belongs on the Expense review status, not a standalone routing task. |
