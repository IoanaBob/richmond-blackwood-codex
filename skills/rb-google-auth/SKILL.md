...
- Gmail draft helpers use `gcloud auth application-default login --client-id-file=<path> --scopes=...` because Google's default gcloud OAuth client can be blocked for Gmail scopes.
...
- Do not send Gmail messages automatically; Gmail helpers create drafts only.

## Helper Behavior

Repo-local Gmail draft helpers default to `--auth-login auto`: try saved gcloud application-default credentials first, then launch browser auth only if the cached token is missing or unusable.
