...
| Source | Scope | Status |
| --- | --- | --- |
| Codex thread instruction, 2026-05-25 | Client BOD document workflow: create/find the client Board Meetings Drive subfolder first, draft agenda there, draft minutes from meeting extracts, produce Word/PDF signing copies, upload signed copy to the same folder, and use AGL/CBMAX Drive patterns as examples | Routed to `skills/rb-client-board-docs/SKILL.md`, `skills/index.md`, source/import logs, and memory; provisional pending first full run using the skill |
