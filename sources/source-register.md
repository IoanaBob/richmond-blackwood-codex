...
| Source | Scope | Status |
| --- | --- | --- |
| Codex thread instruction, 2026-05-25 | User instructed live call initiation in the order TPL, then CB MAX, then MHL; user also supplied CB MAX Friday 2026-05-22 Finanzamt callback context about a letter sent to the Dublin registered address, Hamburg registration/nil-return requirements before deregistration, and missed German-language specifics. | Routed to live Notion calls `RBCALL-26`, `RBCALL-13`, and `RBCALL-24`; repo memory updated as provisional until each call completes and the Finanzamt outcomes are verified |
| Local Codex session `019e400d-033b-7671-851c-f9207a10acb5`, 2026-05-19 | TPL/TECHPACITO LIMITED German Finanzamt/ELSTER draft message: incorporation details, complete VAT/corporation/trade-tax registration clarification, existing Steuernummer nuance, missing ELSTER activation/access code, and request to enable official Finanzamt communication | Routed to live Notion `RBCALL-26`, TPL Communications log, and `clients/Companies/TPL/`; provisional until Finanzamt response is verified |
| Codex thread instruction, 2026-05-22 | TPL/TECHPACITO LIMITED Finanzamt follow-up call setup after MHL, including VAT-number status, 2026-05-19 ELSTER outbound proof, employer/employee registration query, address/tax-number context, and sequencing behind MHL/ZENA `RBCALL-24` | Routed to live Notion Calls/Communications and `clients/Companies/TPL/`; provisional until the call completes and the Finanzamt response is verified |
