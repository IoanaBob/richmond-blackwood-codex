...
| Source | URL | Status |
| --- | --- | --- |
| Xero MCP server | `https://github.com/XeroAPI/xero-mcp-server` | Used for optional per-client Xero MCP setup guidance; provisional until RB verifies each client-specific custom connection |
...
| Source | Scope | Status |
| --- | --- | --- |
| Codex thread instruction, 2026-05-25 | Xero MCP setup for RB; user chose per-client Xero MCP entries, `.env.example` employee setup placeholders, no scope override in v1, and mandatory client reference such as `AGL` before Xero skill use | Routed to `setup/mcp/xero.md`, `.env.example`, `skills/rb-xero-mcp/`, setup/process/source-boundary docs, and durable memory; provisional pending live Xero custom-connection verification |
