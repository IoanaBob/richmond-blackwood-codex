...
Status: provisional.
Source: user instruction in Codex chat, WhatsApp MCP setup, fetched Notion Correspondence and Tasks schemas, and existing RB client routing rules.
Imported: 2026-05-05.
...
Use this process for manual WhatsApp monitoring of a specific saved client chat. The workflow checks only messages newer than the client checkpoint, processes inbound attachment correspondence or actionable text requests, creates the required Notion task, posts a Slack notification, and advances the checkpoint only after all required handling succeeds.
...
5. Inspect each new inbound item and classify it:
   ...
   - Text-only actionable request: create a task and Slack notification; do not create a Correspondence record.
...
If there is no matched company or individual, leave the linkage empty for the Correspondence record. Create a task only when the correct related project can still be identified confidently from existing Notion context.

## Correspondence Records

Use the Correspondence data source:
...
- `Type`: `Incoming`
- `Sent/Received On`: WhatsApp received date.
...
- attach all relevant originals to the destination Correspondence record;
...
Create a task for every correspondence item and every text-only actionable request.
...
- `Project`: the Company Project linked to the matched company or individual.
- `Status`: `To Do`
...
Make the task clearly actionable based on the inbound content and summary. If there are files, reference the already uploaded Correspondence record or attachment rather than saving duplicate copies elsewhere.

## Slack Notification

After task creation, post to `#rb-client-updates`:
...
If task creation fails, do not post the Slack notification and do not advance the checkpoint.
...
4. Do not imply that a Correspondence record, task, translated file, Slack notification, or checkpoint update exists until it has been verified.
...
- Company-related attachment: link company, create Correspondence, create task, notify Slack, advance checkpoint.
- Personally related attachment for linked individual: link only the linked individual, create Correspondence, create task in the related project, notify Slack, advance checkpoint.
- Personally related attachment for unlinked or ambiguous individual: default to company, create Correspondence, create task, notify Slack, advance checkpoint.
- Text-only actionable request: create task and Slack notification only, then advance checkpoint.
...
Before a live run, verify the live Notion fields still match this process, especially Correspondence file properties, Task labels, task template, people assignment IDs, and project relations.
