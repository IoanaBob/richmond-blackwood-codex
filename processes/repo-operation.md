...
- MCP tools for optional local services such as WhatsApp or guarded LinkedIn when the server is configured and available.
...
- LinkedIn MCP guard proxy for normal LinkedIn reads when configured; do not run the upstream LinkedIn MCP server raw unless deliberately debugging setup. Keep write tools blocked in `read_only` mode unless an exact approved send packet and session/persona gates justify a time-bounded override.
