...
- Stages 10 and 11 are auto-approved for Gmail labels, WhatsApp checkpoints, and Slack source-message triage markers that are within the approved workflow. Slack source markers are limited to adding the `📝` reaction to the source message and posting a source-thread reply with the approved task/record links.
...
Stop despite auto-approval if a packet introduces an unexpected mutation, a new destination, a broad Slack mention, a Slack marker reply that contains anything beyond triage destination links/status, an unresolved data-source mismatch that makes routing unsafe, or a source/checkpoint action outside this process.
...
Slack source markers are separate from the human-facing Slack closeout. When a source Slack message has been logged or triaged, add `📝` to that source message and post in its thread where it was routed. The `📝` reaction means "logged/triaged; do not process again." The source-thread reply should use the approved task or record links, for example `Triaged into: <task/record links>.` If no task is needed, use `Logged into: <Communication/Expense/Invoicing row>; no task needed.`
...
The human-facing Slack closeout must be grouped by record type and task linkage, in this order:

1. `Communications without tasks`
2. `Communications with tasks`
3. `Expenses`
4. `Invoices`
5. `Needs checking`
6. `Replies coming up`
...
Use `Invoices` only for new or updated Invoicing rows, not communications that merely mention invoices. In `Communications with tasks`, every bullet must include the linked task URL(s) by name as well as the Communication link. Keep route disputes or low-confidence routing out of the Slack body and in the packet until a separate approved update fixes the underlying record.
...
- Hyperlink all items in the communication, expense, invoice, reply, and blocker sections.
