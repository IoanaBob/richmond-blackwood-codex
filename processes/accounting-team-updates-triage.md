...
3. **Routing Plan**: answer-first packet with a top summary, then human-readable approval tables grouped as Creates, Updates / comments, Skips / no action, and Unresolved / needs decision. Targets must be readable hyperlinks. Omit redundant columns such as `Decision` when the group heading already supplies it, and omit empty/repeated blocker columns. Exact schema/property write payloads, verified assignee Slack mentions, dedupe evidence, and other execution details must be preserved in a later machine log or linked handover file.
