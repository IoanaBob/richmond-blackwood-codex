...
| 2026-05-05 | `rb-task-pr` | Recover VUN client backup and individual routing | Upstream main added the authoritative branch/PR workflow while VUN work was still local. | Created `codex/vun-client-routing-backup` from `origin/main`, reapplied task-owned work, and prepared scoped commits. | `git diff --check`, cached diff check, manifest JSON parse, evidence count, and typecheck passed; push pending; newly created process files remain stashed by user instruction. |
