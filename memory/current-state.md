...
- Optional Xero MCP setup guidance with one static `xero` MCP server entry, active-client selection, RB OAuth app credentials in the base checkout `.env`, per-client ignored local OAuth token storage, and `rb-xero-mcp` skill.
...
Xero MCP is optional and local-only. Use one static Codex MCP server entry named `xero`; select the active client locally before reload with `setup/mcp/select-xero-client.sh <CLIENT_REFERENCE>` or `RB_XERO_ACTIVE_CLIENT_REFERENCE` in `.env`. RB OAuth app credentials live once in the base checkout `.env`, while each selected client gets its own ignored local OAuth token. Every Xero request must include the exact RB client reference, such as `AGL`, and the first Xero MCP call must verify the organisation read-only before any other read or write. Xero client IDs, client secrets, bearer tokens, OAuth tokens, MFA codes, and personal Codex config must not be committed.
