...
- Communications rule: draft outbound communication in chat, always show sender identity, always show `Subject` and source/reply thread for email, prefer replying in existing email threads, and send directly after approval. Store only formal client, workflow, or durable business communications in canonical Communications (`https://www.notion.so/1b5e4130131480ab84f3cca356736807`). Individual-directed internal messages are never logged to Notion.
...
Normal outbound communication should not use software drafts. Compose in chat, show the sender identity, always show `Subject`, source mailbox, and source/reply thread for email, prefer replying in existing email threads, and send directly through the supported connector/MCP tool after approval. Store only formal client, workflow, or durable business communications in canonical Communications (`https://www.notion.so/1b5e4130131480ab84f3cca356736807`, data source `collection://1b5e4130-1314-8183-afd8-000b6f4da982`). Individual-directed internal messages, including one-off Slack/WhatsApp/DM/channel pings to a named teammate, quick questions, reminders, nudges, or coordination notes, are never logged to Notion; use an owning task-capable row instead when follow-up is needed. The old RB Communications database (`https://www.notion.so/c931b1b88ff6412a96c74bd9933da19c`) is migration source only.
...
- Individual-message Notion logging rule: Status: approved by user instruction. Source: user instruction in Codex chat on 2026-06-02. Imported: 2026-06-02. Review: individual-directed internal messages must not be logged to Notion; remove or invalidate any accidental Notion logs and update the skill/memory before continuing.
