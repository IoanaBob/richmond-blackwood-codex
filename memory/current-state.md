...
- Ioana RB sender signature rule: when the exact approved Gmail sender is `ioana@richmondblackwood.com`, use Ioana's saved Gmail signature `ioana general`, not a manual `Best, Ioana` or typed-name-only signoff. Status: approved. Source: user instruction and Gmail settings read-back on 2026-06-10. Imported: 2026-06-10. Review: re-read Gmail settings if the saved signature changes.
...
- Germany growth Reddit reset and cap rule: Status: approved. Source: user instruction in chat on 2026-06-09 after reporting the previous Reddit account was banned. Imported: 2026-06-09. Review: apply before every Reddit run. Treat the prior Reddit account's tasks, Growth Messages, chats, and follow-ups as historical-only/no-follow-up unless the user explicitly approves a specific exception after seeing the old source context and new-account risk. New Reddit flow must verify the active account belongs to Ioana before any send. Public Reddit posts/comments are capped at 3 per day for the active account, spaced at least 15 minutes apart; the gap includes only passive scrolling, reading, rules/style review, and non-sending prep. The cap is a maximum, not a quota to force.
