...
- Optional Xero MCP setup guidance with one static `xero` MCP server entry, active-client selection, per-client custom connection credentials in local `.env`, and `rb-xero-mcp` skill.
