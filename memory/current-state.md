...
- Germany growth LinkedIn persona ownership rule: Status: approved. Source: user instruction in chat on 2026-06-12. Imported: 2026-06-12. Review: apply before every LinkedIn invite, first-message, reply, and follow-up packet. Growth Targets `Owner` is the contact owner; Ioana contacts stay owned by Ioana and Eran contacts stay owned by Eran. Sender switches do not clear the other persona's contacts. Active send queues and quota counts filter by owner, but conflict detection runs globally across Growth Targets and Growth Messages using normalized LinkedIn profile URLs, visible thread/message URLs, message IDs, and name plus current company/location when URL evidence is missing. If the same person is already owned by another persona with any request, connection, message, reply, active follow-up, or blocker, block outreach from the second persona and produce a handoff/reassignment packet for approval.
...
- Germany growth Reddit reset and conservative cap rule: Status: approved. Source: user instruction in chat on 2026-06-09 after reporting the previous Reddit account was banned; updated by user instructions on 2026-06-10 and 2026-06-12. Imported: 2026-06-09. Updated: 2026-06-12. Review: apply before every Reddit run. Treat the prior Reddit account's tasks, Growth Messages, chats, and follow-ups as historical-only/no-follow-up unless the user explicitly approves a specific exception after seeing the old source context and new-account risk. Current Reddit flow must verify the active account belongs to Eran before any send; current account is `Ornery-Service3272`. Public Reddit posts/comments use a conservative cap of 1 approved public post/comment per quota day for the active account until the user explicitly raises it. The prior ramp formula is superseded. Revisit any increase only after at least 3 full calendar days of safe single-comment operation, and still require explicit user approval before changing the cap. Public posts/comments remain spaced at least 15 minutes apart; the gap includes only passive scrolling, reading, rules/style review, and non-sending prep. The cap is a maximum, not a quota to force. Targeting priority is very recent, within the last 3 days, slightly desperate Germany freelancer/support posts involving consulting, freelance/self-employed setup, tax, invoicing, pension/social security, false self-employment, company setup, EOR/payroll, banking/KYC, or foreign-client remote work.
