...
- Optional Xero MCP setup guidance with one static `xero` MCP server entry, active-client selection, per-client OAuth app credentials in the base checkout `.env`, ignored local OAuth token storage, and `rb-xero-mcp` skill.
