...
- Accounting Team Updates triage rule: keep the weekday 11:00 Accounting Team Updates pass separate from inbound communication follow-through; process the current working day's blockers/action points only, treat `New client inbounds` as observed / out of scope, and run the mandatory packet workflow from `skills/rb-accounting-team-updates-triage/SKILL.md`. Stage 1 auto-continues only from clean/no-conflict git state. Stage 2 reads yesterday's and today's Richmond Blackwood Accounting Team Updates, bounded human Slack context, current-day Richmond Blackwood meeting notes through `skills/rb-meeting-notes-action-points/SKILL.md`, and the full active task inventory assigned to RB team users from `internal/people-roles.md`. Stage 3 applies both the meeting-notes skill and `skills/rb-accounting-team-updates-routing/SKILL.md` to produce a Team Updates fill/action plan, match saved meeting context against active RB team tasks for possible context appends, and produce atomic create/update/comment/skip/unresolved routing rows, resolve owning operational rows and client projects before Central Tasks, verify target schema/write-back method, and keep unclear rows unresolved until Stage 4 approved write-back. For the fill plan, yesterday's `Action points` section is the carryover baseline: put prior action points with completion/handling evidence into today's `What was achieved yesterday?`, and carry incomplete prior action points into today's `Action points`; do not copy yesterday's checked `What was achieved yesterday?` rows forward. If the full active RB task inventory cannot be pulled, Stage 3 must state that context matching is incomplete and avoid unsafe creates/appends. The Stage 5 `#rb-client-updates` closeout is built only from verified Stage 4 results and includes task hyperlinks plus verified assignee Slack tags for each created/updated/commented task; unresolved assignee mentions block Stage 5 unless the operator approves a plain-name no-notification fallback.
