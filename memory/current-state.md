...
- Drive search on 2026-06-08 found strongest folder candidate `08. Everguard Research` / `https://drive.google.com/drive/folders/1DTsSxkmjpdA-ZykGCr88nX8iHp82W2hj` under `01. Internal Companies`; related EVG loan-register and duplicate/legacy folders are recorded in `drive-locations.md`.
- High-signal current checks: company purpose/name/email mismatch after Everguard Properties -> Everguard Research change, stale tax registration statuses, VIES `VEIS` typo, project company-relation mismatch, May EVG/Luiza loan invoice linked to KONVI, BOI account-name verification, M&A Arts submissions not yet reviewed, Techpacito register/coupon/referral schedule review, Everguard offering research, and Phillips payable/payment verification.
