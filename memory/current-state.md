...
RB open-banking infrastructure is provisional. Entity bank connectivity is tracked by Notion Company `Reference` in `internal/bank-connectivity-registry.md`, not by internal/external classification. Each company entity should be trackable for Wamo, Bank of Ireland Business, and Wise. The repo-local MCP setup is `setup/mcp/rb-open-banking.md` with server files under `setup/mcp/rb-open-banking/` and skill `skills/rb-open-banking/SKILL.md`. Live provider calls require explicit approval and an `approvalPurpose`; raw credentials, tokens, private keys, authorization codes, session IDs, account IDs, IBANs, balances, transactions, statement exports, and raw bank data stay in `.env.local`, `.codex-local/rb-open-banking/`, provider portals, Notion, Drive, or bank systems, not git.
