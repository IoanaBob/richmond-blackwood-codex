...
Standing call-initiation rule: when the user asks Codex to initiate, place, start, or retry calls, first use `skills/rb-authority-call-setup/SKILL.md` to verify the live Call records and then attempt the saved n8n/ElevenLabs `RB Calls Voice Execution` route when n8n MCP is configured and production execution is allowed. If direct execution is unavailable or policy-blocked, set or verify the earliest valid `Next Call At`, rely on the active 15-minute scheduled runner, and report the exact blocker plus expected pickup window. For ordered sequences, hold later calls out of eligibility until the previous call reaches a terminal state.
