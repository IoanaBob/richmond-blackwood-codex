...
- Communications rule: draft outbound communication in chat, always show sender identity, always show `Subject` and source/reply thread for email, prefer replying in existing email threads, and send directly after approval. Always ask whether the exact sent outbound message should be recorded. Do not create a Notion Communications record unless the user explicitly says to record it. Recording is limited to necessary third-party outbound audit/source records; internal messages are never logged to Notion.
...
- Common tasks follow-through rule: use `skills/rb-common-tasks-follow-through/SKILL.md` for Gmail/WhatsApp-triggered task follow-through. Inventory open task-capable rows across RB Client Databases first, then discover/read communications, ask before recording any sent outbound message, route finance and operational items to the owning task-capable data source, close or advance open work, handle reply plans, and prepare Slack closeout only after task closeout. Recording is limited to necessary third-party outbound audit/source records explicitly approved by the user; internal pings and workflow closeouts are excluded from Notion logging. Stage 1 starts by running `git pull origin main` from the active repo/worktree. Stages 1, 2, 10, and 11 are standing auto-approved within normal scope; after the operator approves the exact Stage 12 Slack closeout text for sending, Stages 13 and 14 are auto-approved for send/closeout only, not for recording. Ioana-authored or Ioana-approved Slack templates are final; for client follow-through, use the latest available Ioana `#rb-client-updates` template exactly, currently `New Correspondence`, `Received invoices`, and `Updated tasks`, and stop if the template or required links/mentions cannot be resolved.
...
Normal outbound communication should not use software drafts. Compose in chat, show the sender identity, always show `Subject`, source mailbox, and source/reply thread for email, prefer replying in existing email threads, and send directly through the supported connector/MCP tool after approval. Always ask whether the exact sent outbound message should be recorded in canonical Communications (`https://www.notion.so/1b5e4130131480ab84f3cca356736807`, data source `collection://1b5e4130-1314-8183-afd8-000b6f4da982`). Do not record unless the user explicitly says yes. Recording is limited to necessary third-party outbound audit/source records. Internal messages, including Slack/WhatsApp/DM/channel pings to a named teammate, quick questions, reminders, nudges, workflow closeouts, or coordination notes, are never logged to Notion; use an owning task-capable row instead when follow-up is needed. The old RB Communications database (`https://www.notion.so/c931b1b88ff6412a96c74bd9933da19c`) is migration source only.
...
- Outbound recording decision rule: Status: approved by user instruction. Source: user instructions in Codex chat on 2026-06-02. Imported: 2026-06-02. Review: always ask whether to record a sent outbound message; only record necessary third-party outbound messages with explicit user approval; never log internal messages or workflow closeouts to Notion.
