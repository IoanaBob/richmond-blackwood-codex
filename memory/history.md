...
- User request: Use the personal tax filing workflow for Celine/Selin German personal taxes, open ELSTER for user login, check ready years, proceed accordingly, and save drafts for review.
- Context read: Repo Selin personal-tax memory, live ELSTER account after user login, Selin 2024/2025 workbook summaries and local source registers, and official ELSTER 2025 Anlage N home-office guidance.
- Actions taken: Saved a 2024 ELSTER ESt 1 A draft after validating payroll prefill, entering 210 home-office days, retaining official health/care certificate values, removing stale transferred `Anlage S`, and running validation/calculation. Started and saved a 2025 ELSTER ESt 1 A draft with main form, Anlage N, Anlage Vorsorgeaufwand, and Anlage Sonderausgaben; retained official payroll/private health prefill and entered 210 home-office days.
...
- User request: Review pasted reservations about the Selin/Celine ELSTER drafts, determine whether the drafted positions can be justified, and prepare any future ELSTER changes while ELSTER is unavailable for this account.
- Context read: User-pasted ELSTER review list, Selin local personal-tax files, live Google Sheets read-backs for the 2024 and 2025 `Summary`, `Tax Analysis`, `Deductibles`, `Revenue`, `Checks`, and `Missing Info` tabs, and official BMF/ELSTER guidance for health/care insurance and home-office daily allowance.
- Actions taken: Added a ready-to-apply ELSTER review packet to Selin's personal-tax records and open questions.
- Decisions made: Treat health/care insurance as the only material ELSTER-value decision. Keep payroll and home-office as drafted unless certificate/day-count evidence changes. Do not enter the 2024 EUR 9,785.30 health/care workbook amount unless insurer bill/payment evidence and the basic health vs mandatory care split are available. Reconcile the 2025 workbook EUR 0 health/care position with the ELSTER official prefill EUR 4,102 before final submission.
- Verification: Google Sheets live read-back confirmed the workbook values and remaining review flags. ELSTER was not opened or changed because another run is using it.
- Limitations or gaps: Await operator approval and Celine/Selin ELSTER login before applying any changes. Health/care evidence and final confirmations for investments, expenses, credits, and direct tax payments remain open.
