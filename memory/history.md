...
- Context read: RBL Notion company record, RBL project, linked filing registrations, selected filing/payment rows, active finance tasks, Client Notes/Document records, CRO Open Data, existing RBL Workhub memory, and internal RB product/role/process memory.
- Actions taken: Created the normal RBL company-memory surface under `clients/Companies/RBL/`, preserving the existing Workhub invoice-validation rule and adding company overview, authority, tax/filings, accounting/payroll, communications, project, Drive, Notion backup, linked-individuals, open-questions, history, and source-register context.
- Decisions made: Treat RBL as both internal operating company and service-provider/counterparty across client contracts; keep client-specific pricing, contract performance, and private client facts in the relevant client folders/live rows; do not store bank-account numbers, card details, credentials, or raw private-channel material in git.
- Verification: Notion context was reviewed through targeted fetches from the prior run state; CRO Open Data was queried directly for company number 735818. No live Notion, Drive, Gmail, Slack, or WhatsApp records were modified during this pass.
- Limitations or gaps: Confirm whether to write the CRO registered office back to Notion, whether April 2026 PAYE payment is paid, whether Payroll/VIES/Corporation Tax `Overdue` statuses are stale, the RBL canonical Drive folder, and the current Sage/Xero migration decision.
...
## 2026-06-09 - EVG Review Corrections

- User request: Apply EVG open-question answers, commit, push, open/update the PR, and merge it.
- Context read: EVG repo memory, live Notion company/filing/invoice/task readbacks, Notion data-source schemas for filing registrations and invoices, and current PR status.
- Actions taken: Updated/read back live Notion company `Business Purpose in Detail`, Annual Returns registration `Registered`, VIES title/status, RTD status, and May EVG/Luiza invoice `Company` relation to EVG. Updated EVG repo memory and durable source/open-question/current-state logs.
...
## 2026-06-12 - MHL Clodagh Notary Fee Proof Submission

- User request: Submit the MHL notary invoice and proof of transfer, then corrected that the invoice was already paid by Richmond Blackwood, that Johnpaul submitted it, and that Simoneta should not be messaged further.
- Context read: User-provided local PDFs, MHL company memory, Notion Expenses and Communications schemas, MHL Drive folder, RB communication/file-upload/google-auth skills, Google persona memory, and Simoneta Slack/Notion user lookups.
- Actions taken: Uploaded the invoice and Wise transfer proof to MHL Drive; created Notion Expense `https://www.notion.so/37de413013148160828dc3a08ca79df7` with amount EUR 162.50, `Status = Paid`, `Type = Paid`, MHL company relation, `Internal company = RICHMOND BLACKWOOD LTD`, Simoneta assignee, and `Submitted by = Johnpaul Okolie`; corrected the submitted-by and paid-by-RB fields after user instruction. A Slack DM to Simoneta was sent before the correction and logged as closed Communication `https://www.notion.so/37de4130131481d89afef68a0a54e5a8`; no further payment follow-up should be sent for this item.
- Decisions made: Route the invoice as an MHL company expense because the notary services were for Mindharbour documents; preserve bank account identifiers only inside the source PDFs, not in repo or Notion text.
- Verification: Drive metadata read-back confirmed invoice file `1hPe68W7GoNHcieK_e8ZyIUXKP-Ldhxl0` and proof file `14VZrozN7IgEPPNMPnLsBIrVmjjIhhfhZ` in MHL folder `17gpcVLGtX3dhwSaLYelvw6m-dSVCHiQs`; Notion read-back confirmed paid status, evidence attachments, MHL relation, RB internal-company value, Simoneta assignee, and Johnpaul submitted-by field.
- Limitations or gaps: The user corrected the Slack direction after one DM had already been sent; do not send further Simoneta follow-up for this paid item unless the user explicitly reopens it.
