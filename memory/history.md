...
## 2026-05-31 - RB Multi-Entity Open Banking Infrastructure

- User request: Implement multi-entity open-banking setup for every legal entity/client, regardless of internal/external classification, with Wamo, Bank of Ireland Business, and Wise tracked separately.
- Context read: existing RB client/bank-account routing rules, `personal-codex` Enable Banking MCP source on `origin/main`, provider docs for BOI Open Banking, Enable Banking linked accounts, Wise profiles/balances/account details/statements, and Wamo Berlin Group PSD2.
- Actions taken: Added provisional `rb-open-banking` MCP infrastructure with provider modules for Enable Banking and Wise plus Wamo direct fallback status, safe registry `internal/bank-connectivity-registry.md`, setup docs, env schema, company bank-connectivity template, and repo-local skill.
- Decisions made: Entity scope is Notion Company `Reference`; internal/external classification is not a bank-connectivity dimension. Live provider calls require explicit approval and `approvalPurpose`. Raw credentials, tokens, authorization codes, session IDs, account IDs, balances, transactions, and statement exports stay out of git.
- Verification: `node --check setup/mcp/rb-open-banking/server.mjs`, `npm run mcp:rb-open-banking -- --status`, `npm run typecheck`, `git diff --check`, and `git ls-files .codex-local .env.local` passed. Safe status reported 13 company entities, 39 registry rows, and no missing required bank-rail rows.
- Limitations or gaps: No live provider credentials, consent flows, ASPSP lookups, Wise profile reads, balance reads, transaction reads, or statement reads were run during infrastructure implementation.
