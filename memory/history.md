...
- Verification: Mein ELSTER showed two drafts after the run. The 2024 draft passed validation with preliminary refund EUR 58. The 2025 draft was saved as the latest draft but still has a `Bankverbindung` validation error.
- Limitations or gaps: Neither return was submitted. Google Sheets and Notion connector calls returned revoked OAuth token errors, preventing live workbook/readback/task writeback. The 2025 draft uses `Oezkohen` transliteration pending reviewer correction to `Özkohen` if ELSTER allows it.
