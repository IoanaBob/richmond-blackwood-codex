...
## 2026-06-03 - Johnpaul Client Task Status Pass

- User request: Treat the active operator as Johnpaul Okolie, check RB client updates, tax-task mentions, Johnpaul assignments, current work state, and update Notion statuses/comments where needed.
- Context read: `AGENTS.md`, `internal/people-roles.md`, `skills/index.md`, `skills/rb-common-tasks-follow-through/SKILL.md`, `skills/rb-personal-tax-analysis-de/SKILL.md`, `skills/rb-memory-capture/SKILL.md`, Notion Tasks schema, Personal Tax Filings schema, and targeted Notion task/filing pages.
- Actions taken: Used Notion connector search/fetch/comment/update routes. Corrected Nathan 2025 Personal Tax Filing `https://www.notion.so/342e4130131480849114fe02636b759a` to `Filed`; set Andrei/NACV TK follow-through `https://www.notion.so/35fe41301314810a8d28eb74a1d96c21` to `Blocked` with 2026-06-11 follow-up after full comment readback showed Johnpaul sent docs to TK and is waiting for TK; marked internal Slack MCP support task `https://www.notion.so/36de41301314815ea9bae55f6db88cb9` `Done`; added clarification comments on Aaron 2025 filing task, Byron 2025 filing task, consolidated RB Clients VAT task, CVA corp-tax ambiguity, and CBMAX May letter review.
- Decisions made: Did not reopen filed Aaron/Byron 2025 tasks because their Personal Tax Filing rows are already `Filed` with submission evidence. Kept SVL prepayment/VAT, EVG bank-name, EXV Workhub/Camden, and Konvi LuxTrust open because no completion evidence surfaced. Treated CVA corp tax as an ambiguous mention rather than Johnpaul-owned status work because assignment sits with another owner and remaining scope is unclear.
- Verification: Targeted Notion readbacks confirmed Nathan 2025 filing `Status = Filed`, Andrei/NACV TK `Status = Blocked` and due date `2026-06-11`, and Slack MCP support `Status = Done`. Notion SQL/data-source query tool was unavailable; discovery was connector-search based, so the scan is not an authoritative all-row inventory.
- Limitations or gaps: No local `NOTION_ACCESS_TOKEN` was available for exhaustive REST inventory. The pass relied on Notion connector search and targeted fetch/readback, so there may be additional Johnpaul assignments not surfaced by search.

## 2026-06-03 - Johnpaul RB Client Updates Task Pass Correction

- User correction: The intended source was task links posted in Slack `#rb-client-updates`, not only tax tasks or broad Notion search.
- Context read: Slack `#rb-client-updates` recent history and Johnpaul mentions from 2026-05-20 through 2026-06-03, Notion Tasks schema, and targeted task/readback pages linked from those Slack posts.
- Actions taken: Set `SVL - set up company ELSTER access` `https://www.notion.so/373e413013148102a7eccf8e53dd2505`, `Kristjan - set up individual/freelancer ELSTER access` `https://www.notion.so/373e41301314819c9e05ef2a7d95b0af`, and `AKS - Activate ELSTER account before activation data expires` `https://www.notion.so/364e4130131481a49833f218dcc08bd3` to `Blocked`. Added status-pass comments to TPL Minijob demand, CLV investing/capital-gains answer, EIP AnySocial GPCC/hearing follow-up, Teresa 2024 tax-liability answer, Gabriel 2025 filing, AGL VAT filing, Monochromatic contractor contract, VUN/Nathan VAT/insolvency, and CLV payroll-tax enforcement rows.
- Decisions made: Left `Done` rows closed where proof/readback existed, including CBMAX ROS direct debit notification, CBMAX 2024/2025 zero German returns, Aaron 2025 trade-tax/VAT correction, authorized-wording confirmation, TPL/Barden direct-contract route, and WEW CLLM/tax-liability answer. Left open `To Do` rows open when no completion evidence surfaced instead of inferring completion from the Slack closeout.
- Verification: Notion readbacks confirmed `Blocked` on the two SVL/Kristjan ELSTER access rows and the AKS ELSTER activation row. Slack readback established the source channel/message scope. Notion SQL/data-source query still failed internally, so the pass was targeted from Slack task links rather than an exhaustive database query.
- Limitations or gaps: Older `#rb-client-updates` posts with no Johnpaul mention, pure cc-only tax-prepayment notices, or links outside the Tasks/filing scope were not treated as owner tasks.
