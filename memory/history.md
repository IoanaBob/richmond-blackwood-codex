...
- Actions taken for follow-up: Moved Audiences, Channels, Targets, Compliance Checks, and the then-active summary table under top-level `RB Client Databases` and updated the Germany Growth project page to task-only scope copy. The summary table was later superseded by timestamp-based reporting and trashed.
...
- Actions taken for follow-up: Added LinkedIn intra-day run modes for invite batches, acceptance checks, first-message packets, follow-up sweeps, reply triage, and reporting-only closeout.
...
- User follow-up on 2026-06-01: Corrected that "How are you thinking about that?" is not a specific question, and tax should only appear as a passing joke: "Admin and taxes I'm sure were much easier in the States."
- Actions taken for follow-up: Tightened LinkedIn guidance so the tax/admin remark is only a light aside, and questions must be highly pointed decision/tradeoff questions tied to a named product, market, technical, or founder signal.
- User follow-up on 2026-06-01: Corrected that "saw your work" is broad and means nothing, and should never be used.
- Actions taken for follow-up: Blocked "saw your work" in the LinkedIn guidance and replaced the remaining technical-role example with a named project/system/post/launch pattern.
...
- User follow-up on 2026-06-01: Add Reddit DMs when there is engagement with our post/comment or when we receive direct DMs.
- Actions taken for follow-up: Updated the Reddit skill, master skill, live Notion Reddit channel row, Reddit identity check, and Communications growth event options so cold/proactive Reddit DMs remain blocked, while reactive DMs are allowed after public engagement or inbound DM and still require exact approval plus immediate Ioana Reddit-session verification.
- User follow-up on 2026-06-01: Facebook group partnership acquisition and Facebook group posting were mixed and should probably be two different skills.
- Actions taken for follow-up: Split the old mixed Facebook Groups skill into `rb-germany-growth-facebook-partnerships` for admin/sponsorship/payment/commercial approval acquisition through Business Partners and `rb-germany-growth-facebook-posting` for public group posts/comments/replies through Growth Targets and Communications. Updated the master skill, skill index, durable memory, and live channel rows so the two workstreams run separately.
- User follow-up on 2026-06-01: Relocation partner outreach should have a daily target and open at least 5 new first-time email conversations per day.
- Actions taken for follow-up: Added a 5/business-day first-time email conversation target to the relocation partner skill, master daily skill, durable operating notes, and live relocation channel row. Replies and follow-ups are tracked separately and do not count toward that target; all first-time email sends still require exact approval and immediate Ioana sender-session verification.
- User follow-up on 2026-06-01: Remove the dedicated growth Metrics table and reconstruct reporting from timestamps on the underlying records instead.
- Actions taken for follow-up: Added timestamp fields to Audiences, Channels, Growth Targets, Business Partners, Communications, and Compliance Checks; removed the Metrics relation from Audiences; trashed `RB DE Growth Metrics`; and rewrote the master/channel skills to use timestamp-based reporting instead of summary rows.
- User follow-up on 2026-06-01: Remove the Compliance Checks database because it is a one-time action done as part of each skill, not something that needs its own database.
- Actions taken for follow-up: Trashed `RB DE Growth Compliance Checks`, removed it from active operating sources, and rewrote the master/channel skills so compliance checks happen as in-run gates. Blockers and follow-ups now route to Communications or Tasks; rule and approval evidence stays on the relevant Growth Target, Business Partner, or communication record.
