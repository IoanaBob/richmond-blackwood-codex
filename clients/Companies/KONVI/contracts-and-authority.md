# Contracts And Authority

Status: provisional.
Source: Notion company record, linked contract records, shareholder company record, and user instruction on 2026-05-26.
Imported: 2026-05-26.
Review: Confirm active vendor/support contract scope and reconcile stale shareholder-company placeholders before using this file for outbound authority statements.

## Authority And Governance

KONVI LIMITED is recorded in Notion as Irish incorporated and Irish tax resident. Eran Peer is recorded as director and UBO. KONVI HOLDINGS LIMITED is recorded as shareholder company.

The company page records previous names `KONVI FIVE YEAR WATCH HOLDINGS 001 LTD` and `KONVI CROWDFUNDING LTD (until Q4 2024)`. The Notion document `Konvi Name Change - Requirements` says the change from Konvi Crowdfunding Limited to Konvi Limited did not change legal identity, rights, obligations, contracts, debts, or liabilities, but tax records, ROS, bank accounts, insurance policies, financing agreements, partners, and invoices should be updated to the new name.

Shareholder-company source: `KONVI HOLDINGS LIMITED` / `https://www.notion.so/e4f186fded244b34abfdf572f7bbd55d`.

## Active / Current Contracts

| Contract | Status | Counterparty | Amount | Frequency | Source | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| Konvi -> RB Monthly | Active | Richmond Blackwood | EUR 9,500 | Monthly | `https://www.notion.so/33be413013148032907afcac7ba0be3b` | Temporary liquidity arrangement says Konvi pays EUR 6,000 plus VAT until liquidity recovers, then pays back the difference. |
| Head of Growth - Peter Maslik | Active | Peter Maslik | EUR -12,000 | Monthly | `https://www.notion.so/23ee4130131480659cdccfd2dd3796e4` | Czech jurisdiction support/contractor cost. |
| Konvi <> Parabellum (Alex Gulin) | Active | Parabellum / Alex Gulin | EUR -5,000 | Monthly | `https://www.notion.so/24ae41301314800e8decff894183d4fe` | Notion contract end date is 2026-05-31; confirm renewal/termination before future payments. |

## Historical / Inactive Contracts

| Contract | Status | Counterparty | Amount | Source | Notes |
| --- | --- | --- | --- | --- | --- |
| Konvi <> Giuliano | Inactive | Giuliano | EUR -15 hourly | `https://www.notion.so/2ace4130131480fd88b5dbe72bb811db` | Ended 2026-03-31. |
| Konvi <> Dracoon Ventures UG | Inactive | DRACOON VENTURES UG | EUR -4,000 monthly | `https://www.notion.so/318e41301314801da9aec39356e47299` | Ended 2026-04-15. Separate open task says to attach invoice lines for a EUR 500/week Dracoon arrangement, so payment evidence still needs reconciliation. |

## Legal / Regulatory Issues

| Issue | Source | Review needed |
| --- | --- | --- |
| XVault / MiCA / CASP risk review | `https://www.notion.so/2b1e4130131480df9b26fe24faec0862` | Task is In Progress. It records concern that adding a tokenisation/XVault line under existing Konvi Limited could affect the current license, and that CBI suggested liaising with the CASP team. |
| LuxTrust Scan order pack | `https://www.notion.so/364e4130131481aba141fbf1f79f679a` | High-priority To Do task for order-pack completion; confirm entity, signer authority, and submission route before sending documents externally. |
| Banksy buy-side liquidity support package | `https://www.notion.so/360e413013148130b3d3ef6b667f2703` | Marked Done with Ioana correction on 2026-05-14; keep as closed context, not an active blocker. |
