# CBMAX - CBMAX FORGEMATE VENTURES LIMITED

Status: provisional.
Source: Notion company record `https://www.notion.so/c71b32a86f424e63b63945fe63d7b45b`; Drive folder search; Gmail search; WhatsApp chat `Brivio, Claudio | Richmond Blackwood`; user clarifications on 2026-05-13.
Imported: 2026-05-13.
Review: Confirm whether the discovered Drive folder is the canonical long-term CBMAX folder, sync the pending/limbo VAT backdate position back into Notion, and decide whether any personal-tax/person files should be split into individual folders now.

Company reference: `CBMAX`.
Legal name: CBMAX FORGEMATE VENTURES LIMITED.
Service: Managed Services.
Notion company record: `https://www.notion.so/c71b32a86f424e63b63945fe63d7b45b`.
Client project: `https://www.notion.so/32fe413013148034830cf9a1281b2f10`.
Client dashboard: `https://www.notion.so/67a175a2d9f44c55853a64f106b44752?v=f446b59bf86e4c469e5eae001bd9222c`.
Drive folder found: `https://drive.google.com/drive/folders/1QhfuwpdncQdGx6bymJv29rD3K_3yyqjn`.

Notion backup: `notion-backup.md`.
Drive evidence: `drive-locations.md`.
Detailed history: `history.md`.
Linked individuals: `linked-individuals.md`.
Client project context: `client-project.md`.

## Canonical Identifiers

| Field | Value |
| --- | --- |
| Client reference | `CBMAX` |
| Legal name | CBMAX FORGEMATE VENTURES LIMITED |
| Register number | `755006` |
| Registered on | 2024-01-04 |
| Incorporated in | Ireland |
| Tax residence in company property | Ireland |
| Registered office | Workhub, 77 Camden Street Lower, Dublin 2, D02 XE80, Ireland |
| Correspondence address | Kamminer Strasse 2, 10589 Berlin, Germany |
| Company email | `contact+cbmaxforgemateventures@richmondblackwood.com` |
| Accounting software | Lexoffice |
| Accounting software email | `contact+cbmaxforgemateventures@richmondblackwood.com` |
| Monthly fee to RB | EUR 950 |

## Record Routing

Long-lived operational records should live in the matching Notion table and link back to the company when the schema supports it. This includes company information, contracts, employment, payroll/payslips, invoices, expenses, correspondence, tasks, and filing records.

Drive should hold source documents and records that do not fit cleanly into Notion or do not need frequent operational access. In git, keep pointers and routing notes only.

Individual-specific details for Claudio Brivio, Eran Peer, Semen, Viktor, or other linked people should be placed under `clients/Individuals/<legal-name>/` before importing personal tax, KYC, personal identifier, personal bank, personal asset, or personal correspondence detail.

## Current High-Signal Context

- Latest WhatsApp plus user-correction context puts CBMAX's 2025 VAT/tax-residence treatment in limbo pending ROS acceptance of the retroactive Irish VAT position. Irish VAT number `4388950KH` was issued and ROS backdate was requested from January 2025, but the user clarified on 2026-05-13 that retroactive acceptance was not yet confirmed.
- The 2026-03-30 WhatsApp guidance about using Irish VAT and not German VAT for 2025 purchases should be treated as pending/conditional until ROS accepts the backdate.
- The older Notion tax-history callout still contains German/Ireland wording that needs cleanup, but should not be treated as the live 2025 position without review.
- German VAT records include `DE451021099` and USt-IdNr. `DE455085802`; Irish VAT/CT records use `4388950KH`.
- A Hamburg tax-office task says a 2024 estimated VAT demand uses unfamiliar tax number `17/070/40128`, while VAT filings were made at zero under Berlin tax number `29/648/60019`.
- User confirmed on 2026-05-13 that Simon is the CBMAX employee recorded as Semen, with relationship through the Employment table.
