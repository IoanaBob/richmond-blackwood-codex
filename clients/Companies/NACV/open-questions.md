# Open Questions

Status: provisional.
Source: NACV context import on 2026-05-19.
Imported: 2026-05-19.
Review: Close questions only after source evidence is updated in Notion/Drive or the user gives explicit direction.

## Company Tax / VAT

- Confirm Finanzamt response/notice for NACV 2024 and 2025 tax amounts and go-forward prepayment amounts.
- Populate the linked Tax Prepayments database once the Finanzamt amounts and dates are known.
- Confirm the additional information requested by Finanzamt for the filed 2024/2025 company taxes and whether RB's response has been accepted.
- Confirm Finanzamt response to the VAT deregistration appeal/reinstatement request; user says no response yet.
- Confirm whether the monthly German VAT filing cadence can be reduced after reinstatement/compliance is clear.

## Drive / Evidence

- Complete the open Notion task to split Andrei's 2024 personal-tax documents from the mistaken 2025 folder if not already done.
- Confirm whether all NACV payslips and wage-tax records needed for Andrei's personal tax are uploaded to Notion/Drive; otherwise retrieve them from Lexoffice with operator login.

## Communications

- Keep `NA Capital Ventures | RB` / `120363399321589278@g.us` as the selected WhatsApp route pointer, but do not use it for monitoring or sending without explicit approval.
