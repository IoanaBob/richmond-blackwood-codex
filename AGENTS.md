...
- Always ask the user whether a sent outbound message should be recorded. Do not create a Notion Communications record unless the user explicitly says to record that exact message. Only record third-party outbound messages, and only when recording is necessary as a durable audit/source record. Never log internal messages to Notion; this includes one-off Slack/WhatsApp/DM/channel pings to a named teammate, quick questions, reminders, nudges, workflow closeouts, or coordination notes, even when they mention a client or matter. The old RB Communications database is migration source only. If a user-approved required Communications database log is unavailable because the database or schema is unclear, report the blocker and record it in `memory/open-questions.md`.
