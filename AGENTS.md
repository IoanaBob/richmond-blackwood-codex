...
- After sending, store only formal client, workflow, or durable business communications in canonical Communications (`https://www.notion.so/1b5e4130131480ab84f3cca356736807`). Never log individual-directed internal messages to Notion; this includes one-off Slack/WhatsApp/DM/channel pings to a named teammate, quick questions, reminders, nudges, or coordination notes, even when they mention a client or matter. The old RB Communications database is migration source only. If a required Communications database log is unavailable because the database or schema is unclear, report the blocker and record it in `memory/open-questions.md`.
