...
- In pitch decks and presentation packets, match the template's eyebrow structure and capitalization. If the template uses uppercase navigation labels, use uppercase eyebrows.
- When creating slides from a provided template, copy the matching template slide first and edit its existing elements in place. Preserve template typography, color treatment, and component styling. Preserve spacing and layout unless the user explicitly approves a redesign.
- Treat provided presentation templates as source files, not delivery files. Create or duplicate into a separate working deck before producing a client-review deck unless the user explicitly asks to edit the template file itself.
