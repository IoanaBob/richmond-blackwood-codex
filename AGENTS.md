...
- Notion REST/API access should expect `NOTION_ACCESS_TOKEN` in the ignored `.env.local` of the main Richmond Blackwood checkout when it is not already visible in the active process environment or current worktree. Read only that key from the main checkout env file, never print or dump env files, and do not treat a missing shell environment variable as missing credentials until that approved local route has been checked.
